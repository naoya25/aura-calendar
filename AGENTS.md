...
このファイルは、AI がこのリポジトリを触るときの最小限の案内です。迷ったら、まず既存の実装と [README.md](README.md)、[docs/guide.md](docs/guide.md)、[docs/git-workflow.md](docs/git-workflow.md) を確認してください。
...
- [dist/settings.html](dist/settings.html) は設定画面の実体なので、UI の修正はここを直接直す
- `target/` は生成物なので触らない
...
- PR 作成時は [`.github/pull_request_template.md`](.github/pull_request_template.md) を必ず参照する
- テンプレートの「関連Issue」欄は、必要に応じて issue を検索して埋める
- 変更内容は「何を直したか」「どこに影響するか」「どう確認したか」を短く書く
- 関連 issue がある場合は、番号だけでなく何と結びつく変更かも明示する
