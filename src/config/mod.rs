...
const MIN_DISPLAY_INTERVAL_SECONDS: u64 = 1;
...
impl Default for DisplayConfig {
    fn default() -> Self {
        Self {
            normal_format: "{% if active %}now{% else %}{% if d > 0 %}{{ d }}d{% endif %}{% if h > 0 %}{{ h }}h{% endif %}{% if m > 0 %}{{ m }}m{% endif %}{% endif %}|{{ title }}{% if count > 1 %} :{{ count }}{% endif %}".to_string(),
